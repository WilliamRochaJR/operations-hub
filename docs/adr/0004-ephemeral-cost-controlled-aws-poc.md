...
- `iam:GetRole` limitado aos ARNs exatos, sem path e com path, de `AWSServiceRoleForAmazonEKSNodegroup`, pois a validação ocorre antes de a role existir e a leitura posterior usa o path gerenciado;
...
A criação do managed node group pode exigir que a pipeline crie `AWSServiceRoleForAmazonEKSNodegroup`. A condição de `iam:CreateServiceLinkedRole` inclui explicitamente `eks-nodegroup.amazonaws.com`; a role não pode criar service-linked roles para serviços fora da lista aprovada.
